...
COEFICIENTE_RESTITUICAO = 0.4 #coeficiente de restituição

AVOGADRO_CONSTANT = 6.022e23 #constante de avogadro

NAME_BOLTZ = 'max-boltz_6_15_4_no_UI.png'
NAME_CONCENTRATION_PLOT = 'concentration_6_15_4_no_UI.png'
NAME_CHEMICAL_CSV = 'df_chemical_data_6_15_4_no_UI.csv'
NAME_MOLARITY_CSV = 'df_molarity_data_6_15_4_no_UI.csv'
...
def concentration_plot(df_chem_molarity,name):
    # Lineplot das concentrações/tempo.
    fig, ax = plt.subplots(figsize=(6, 6))  # Cria caixa para plot
    for y_axis, labels in zip(df_chem_molarity.columns[1:],list(df_chem_molarity.columns)[1:]):
        plt.plot(df_chem_molarity['Tempo'],df_chem_molarity[y_axis] ,alpha=0.7, label=labels)
    plt.legend()
    plt.xlabel('Tempo [s]')
...
def main():
    ...
    while True:
        ...
        if elapsed_time >= MAX_RUNTIME_SECONDS:
            final_speeds = np.array([math.sqrt(particle_instance.vel_x**2 + particle_instance.vel_y**2) for particle_instance in particles.values()])
            ...
            concentration_plot(df_chem_molarity,name=NAME_CONCENTRATION_PLOT)
            break
